...
function ResultContainer() {
  ...
  let text = params.get('q'); // is the string "Jonathan Smith".

  const API_URL = `${REDIRECT_URI}questions/search?page=1&keyword=${text}`;
    ...
    <div>
      {count === 0 ? (
        ...
        <div>
          {post &&
            post.map((search) => {
              return <Searchitem key={post.searchId} search={search} />;
            })}
        </div>
      )}
    </div>
